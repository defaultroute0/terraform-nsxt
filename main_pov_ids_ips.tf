...
data "nsxt_logical_tier0_router" "PoV-T0-Gateway" {
  display_name = "PoV-T0-Gateway"
}
...
resource "nsxt_logical_router_link_port_on_tier0" "tier0_port_to_tier1" {
  ...
  logical_router_id = "${data.nsxt_logical_tier0_router.PoV-T0-Gateway.id}"
  tag {
    scope = "app"
    tag   = "terraformdemo"
  }
